...
Loads the best checkpoint, runs inference on the full Bay of Bengal test
set, inverse-transforms predictions and targets back to REAL UNITS using
the saved scaler, and computes four validation statistics per forecast
variable (swh, mwp, mwd):
  - Correlation coefficient (Pearson r)
  - Scatter Index (SI = RMSE / mean(actual))
  - Bias (mean(predicted - actual))
  - RMS Error (RMSE, in real units)

Saves: bay_of_bengal/results/validation_metrics.json
...
SCALER_PATH = SEA_DIR / "scaler_bay_of_bengal.pkl"
...
def main():
    print("=" * 60)
    print("EXTENDED VALIDATION METRICS - BAY OF BENGAL")
    print("=" * 60)
    ...
    table_md = (
        "# Extended Validation Metrics — Bay of Bengal\n\n"
        "Computed in real units (inverse-transformed via scaler_bay_of_bengal.pkl), "
    ...
    print("\n[OK] Extended validation metrics complete for Bay of Bengal.")
