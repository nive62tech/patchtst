"""
Phase 4 (Bay of Bengal) - Label Definition (Classification Target)

Loads bay_of_bengal/data/bay_of_bengal_features.csv, computes Bay of Bengal's OWN
quartile-based mwp_class bin edges (independently - not shared with any other
sea, since wave period ranges differ by location), applies them via pd.cut(),
checks class distribution, and saves the dataframe back out with the mwp_class
column added.

Bin edges here are computed FRESH from this sea's own cleaned mwp distribution
via quartile split. Once this run prints the edges, they should be treated as
CONFIRMED and hardcoded for all future Bay of Bengal runs.
"""
...
INPUT_PATH = DATA_DIR / "bay_of_bengal_features.csv"
OUTPUT_PATH = DATA_DIR / "bay_of_bengal_features.csv"  # same file, mwp_class column added
...
def main():
    print("=" * 60)
    print("PHASE 4 (BAY OF BENGAL) - LABEL DEFINITION (CLASSIFICATION TARGET)")
    print("=" * 60)

    if not INPUT_PATH.exists():
        raise FileNotFoundError(f"{INPUT_PATH} not found - run Bay of Bengal Phase 3 first.")
    ...
    print("\n--- Computing Bay of Bengal's OWN quartile bin edges ---")
