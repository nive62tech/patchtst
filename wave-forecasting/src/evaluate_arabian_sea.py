"""
Phase 12 (Arabian Sea) - Model Evaluation on Test Set

Loads the best Arabian Sea checkpoint, runs inference on the full Arabian
Sea test set (never used in training or validation), and computes
classification and forecasting metrics, plus a year-by-year breakdown.

Saves: arabian_sea/results/confusion_matrix.png, horizon_error_curve.png,
metrics_report.json

Mirrors src/evaluate.py (Atlantic) exactly, renamed for Arabian Sea.
"""
...
CHECKPOINT_PATH = SEA_DIR / "arabian_sea_patchtst_best.pt"
TEST_CSV_PATH = DATA_DIR / "arabian_sea_test_scaled.csv"
...
def main():
    print("=" * 60)
    print("PHASE 12 (ARABIAN SEA) - MODEL EVALUATION ON TEST SET")
    print("=" * 60)

    if not CHECKPOINT_PATH.exists():
        raise FileNotFoundError(f"{CHECKPOINT_PATH} not found - run Arabian Sea Phase 11 first.")
    ...
    plt.ylabel("True")
    plt.title("Sea State Confusion Matrix (Arabian Sea Test Set)")
    plt.tight_layout()
    ...
    plt.title("Horizon-wise RMSE (Arabian Sea Test Set)")
    ...
    print("\n[OK] Phase 12 evaluation complete for Arabian Sea.")
