"""
Phase 3 (Arabian Sea) - Feature Engineering (Direction Encoding)

Loads arabian_sea/data/arabian_sea_clean.csv, applies sin/cos circular encoding to
mwd (Formula 3), keeps a copy of raw mwd in the file for later use as a
Phase 5 forecasting target, and saves arabian_sea/data/arabian_sea_features.csv.
...
Mirrors src/feature_engineering.py (Atlantic) exactly, renamed for Arabian Sea.
"""
...
INPUT_PATH = DATA_DIR / "arabian_sea_clean.csv"
OUTPUT_PATH = DATA_DIR / "arabian_sea_features.csv"
...
def main():
    print("=" * 60)
    print("PHASE 3 (ARABIAN SEA) - FEATURE ENGINEERING (DIRECTION ENCODING)")
    print("=" * 60)

    if not INPUT_PATH.exists():
        raise FileNotFoundError(f"{INPUT_PATH} not found - run Arabian Sea Phase 2 first.")
