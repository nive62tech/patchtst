"""
Phase 5 (Bay of Bengal) - Forecasting Targets
Defines and verifies the target construction logic for the forecasting head,
identical to src/targets.py (Atlantic): given a starting index i (the same
starting index as the Phase 8 input window), builds a sequence of 20 future
values (t+6h, t+12h, ..., t+120h) for swh, mwp, and raw mwd. This module is
imported (logic reproduced inline) by Phase 8's Bay of Bengal windowing
script - it does not itself split the data or produce final .npy files.
"""
import numpy as np
import pandas as pd
...
def main():
    print("=" * 60)
    print("PHASE 5 (BAY OF BENGAL) - FORECASTING TARGETS")
    print("=" * 60)
    if not INPUT_PATH.exists():
        raise FileNotFoundError(f"{INPUT_PATH} not found - run Bay of Bengal Phase 4 first.")
    ...
    print(
        "\n[OK] Target construction logic verified for Bay of Bengal. This module's logic "
        "will be reproduced in Phase 8's windowing_bay_of_bengal.py to build y_forecast arrays "
