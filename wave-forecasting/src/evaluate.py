...
Loads the best checkpoint, runs inference on the full test set (test .npy
files from Phase 8 - never used in training or validation), and computes:
- Classification: accuracy, weighted/macro F1, confusion matrix, per-class
  precision/recall/F1
- Forecasting: MAE/RMSE per forecast step (t+6h to t+120h), R^2, horizon
  error curve
- Year-by-year breakdown across the test period, to check for performance
  drift over time

Saves: results/confusion_matrix.png, results/horizon_error_curve.png,
...
TEST_CSV_PATH = DATA_DIR / "atlantic_test_scaled.csv"
...
FORECAST_STEPS = 20
STEP_HOURS = 6
BATCH_SIZE = 256  # larger batch is fine for inference-only, no gradients held


def get_window_reference_dates(df: pd.DataFrame) -> np.ndarray:
    """
    Reconstructs the same set of valid start indices used in Phase 8's
    windowing, and returns the classification-target date (valid_time at
    i+72) for each window - used for the year-by-year breakdown.
    """
    n_rows = len(df)
...
def main():
    print("=" * 60)
    print("PHASE 12 - MODEL EVALUATION ON TEST SET")
    print("=" * 60)
    ...
    plt.ylabel("True")
    plt.title("Sea State Confusion Matrix (Test Set)")
    plt.tight_layout()
    ...
    plt.title("Horizon-wise RMSE (Test Set)")
    ...
    print("\n[OK] Phase 12 evaluation complete.")
