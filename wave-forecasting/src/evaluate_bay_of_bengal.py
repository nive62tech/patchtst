"""
Phase 12 (Bay of Bengal) - Model Evaluation on Test Set

Loads the best Bay of Bengal checkpoint, runs inference on the full Bay of
Bengal test set (never used in training or validation), and computes
classification and forecasting metrics, plus a year-by-year breakdown.

Saves: bay_of_bengal/results/confusion_matrix.png, horizon_error_curve.png,
metrics_report.json

Mirrors src/evaluate.py (Atlantic) exactly, renamed for Bay of Bengal.
"""
...
DATA_DIR = SEA_DIR / "data"
WINDOWS_DIR = DATA_DIR / "windows"
RESULTS_DIR = SEA_DIR / "results"
CHECKPOINT_PATH = SEA_DIR / "bay_of_bengal_patchtst_best.pt"
TEST_CSV_PATH = DATA_DIR / "bay_of_bengal_test_scaled.csv"
...
def main():
    print("=" * 60)
    print("PHASE 12 (BAY OF BENGAL) - MODEL EVALUATION ON TEST SET")
    print("=" * 60)

    if not CHECKPOINT_PATH.exists():
        raise FileNotFoundError(f"{CHECKPOINT_PATH} not found - run Bay of Bengal Phase 11 first.")
    ...
    plt.ylabel("True")
    plt.title("Sea State Confusion Matrix (Bay of Bengal Test Set)")
    plt.tight_layout()
    ...
    plt.title("Horizon-wise RMSE (Bay of Bengal Test Set)")
    ...
    print("\n[OK] Phase 12 evaluation complete for Bay of Bengal.")
