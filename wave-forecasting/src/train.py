...
Core training loop for the PatchTST dual-head model. Designed to be imported
both locally (for a quick sanity run, CPU, few epochs) and inside the Colab
notebook (notebooks/colab_train.ipynb) for the actual full training run on
GPU - up to 100 epochs with early stopping is not meant to run on the local
CPU-only machine per this project's environment constraints.
"""
...
if __name__ == "__main__":
    # Local sanity run only: a couple of epochs on CPU, to confirm the loop
    # runs end-to-end without errors before moving to Colab for the real run.
    print("=" * 60)
    print("PHASE 11 - LOCAL SANITY CHECK (2 epochs, CPU)")
    print("=" * 60)
