"""
Phase 7 (Bay of Bengal) - Normalization

Fits a StandardScaler on the training set only (bay_of_bengal_train.csv),
applies it to both bay_of_bengal_train.csv and bay_of_bengal_test.csv, and
saves the fitted scaler as bay_of_bengal/scaler_bay_of_bengal.pkl.
mwp_class and valid_time are never scaled.

Scaled columns: u10, v10, swh, mwp, sin_mwd, cos_mwd, mwd (mwd is scaled
because it doubles as a forecasting target alongside swh/mwp).

Mirrors src/normalize.py (Atlantic) exactly, renamed for Bay of Bengal.
"""
...
DATA_DIR = Path(__file__).resolve().parent.parent / "bay_of_bengal" / "data"
SEA_DIR = Path(__file__).resolve().parent.parent / "bay_of_bengal"

TRAIN_PATH = DATA_DIR / "bay_of_bengal_train.csv"
TEST_PATH = DATA_DIR / "bay_of_bengal_test.csv"
TRAIN_OUTPUT_PATH = DATA_DIR / "bay_of_bengal_train_scaled.csv"
TEST_OUTPUT_PATH = DATA_DIR / "bay_of_bengal_test_scaled.csv"
SCALER_OUTPUT_PATH = SEA_DIR / "scaler_bay_of_bengal.pkl"
...
def main():
    print("=" * 60)
    print("PHASE 7 (BAY OF BENGAL) - NORMALIZATION")
    print("=" * 60)

    if not TRAIN_PATH.exists() or not TEST_PATH.exists():
        raise FileNotFoundError(
            "bay_of_bengal_train.csv / bay_of_bengal_test.csv not found - run Bay of Bengal Phase 6 first."
        )
