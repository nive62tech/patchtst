"""
Phase 7 (Pacific) - Normalization

Fits a StandardScaler on the training set only (pacific_train.csv), applies
it to both pacific_train.csv and pacific_test.csv, and saves the fitted
scaler as pacific/scaler_pacific.pkl. mwp_class and valid_time are never
scaled.

Scaled columns: u10, v10, swh, mwp, sin_mwd, cos_mwd, mwd (same 7-column
decision as Atlantic's Phase 7 - mwd is scaled because it doubles as a
forecasting target alongside swh/mwp).

Mirrors src/normalize.py (Atlantic) exactly, renamed for Pacific.
"""
...
TRAIN_PATH = DATA_DIR / "pacific_train.csv"
TEST_PATH = DATA_DIR / "pacific_test.csv"
TRAIN_OUTPUT_PATH = DATA_DIR / "pacific_train_scaled.csv"
TEST_OUTPUT_PATH = DATA_DIR / "pacific_test_scaled.csv"
SCALER_OUTPUT_PATH = PACIFIC_DIR / "scaler_pacific.pkl"
...
def main():
    print("=" * 60)
    print("PHASE 7 (PACIFIC) - NORMALIZATION")
    print("=" * 60)

    if not TRAIN_PATH.exists() or not TEST_PATH.exists():
        raise FileNotFoundError(
            "pacific_train.csv / pacific_test.csv not found - run Pacific Phase 6 first."
        )
