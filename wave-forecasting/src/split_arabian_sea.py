"""
Phase 6 (Arabian Sea) - Train/Test Split - 70/30 by Date

Loads arabian_sea/data/arabian_sea_features.csv, computes a strict
chronological 70/30 split (no shuffling), and saves
arabian_sea_train.csv / arabian_sea_test.csv.

Mirrors src/split.py (Atlantic) exactly, renamed for Arabian Sea.
"""
...
TRAIN_OUTPUT_PATH = DATA_DIR / "arabian_sea_train.csv"
TEST_OUTPUT_PATH = DATA_DIR / "arabian_sea_test.csv"
...
def main():
    print("=" * 60)
    print("PHASE 6 (ARABIAN SEA) - TRAIN/TEST SPLIT (70/30 BY DATE)")
    print("=" * 60)

    if not INPUT_PATH.exists():
        raise FileNotFoundError(f"{INPUT_PATH} not found - run Arabian Sea Phase 5 first.")
