"""
Phase 6 (Pacific) - Train/Test Split - 70/30 by Date

Loads pacific/data/pacific_features.csv, computes a strict chronological
70/30 split (no shuffling), and saves pacific_train.csv / pacific_test.csv.

Mirrors src/split.py (Atlantic) exactly, renamed for Pacific.
"""
...
TRAIN_OUTPUT_PATH = DATA_DIR / "pacific_train.csv"
TEST_OUTPUT_PATH = DATA_DIR / "pacific_test.csv"
...
def main():
    print("=" * 60)
    print("PHASE 6 (PACIFIC) - TRAIN/TEST SPLIT (70/30 BY DATE)")
    print("=" * 60)

    if not INPUT_PATH.exists():
        raise FileNotFoundError(f"{INPUT_PATH} not found - run Pacific Phase 5 first.")
